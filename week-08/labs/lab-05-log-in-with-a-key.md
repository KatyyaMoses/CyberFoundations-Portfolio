...
```text
No, the private key never goes into authorized_keys. Only the public key belongs there. The server uses the public key to check that I'm holding the matching private key, but my private key always stays on my own machine
```
...
```text
In Week 6 I logged in with the analyst account password, and in Week 8 I logged in using my key pair instead. What changed is how I proved my identity, moving from something I type to something I hold, which is harder to steal or guess. What stayed the same is that SSH was still the tool making the secure connection, and the goal was still logging in safely to the machine.
```
...
```text
The method of proving who I was changed. Week 6 used a password, and Week 8 used a private key with a passphrase. The key-based way is stronger because there's no password being sent that could be guessed or intercepted.
```

2. What stayed the same?

```text
SSH was still the program handling the connection, and the point was still to log in securely to a remote machine. The end goal did not change, only the way I authenticated.
```

3. Why does `authorized_keys` contain the public key rather than the private key?

```text
Because the public key is safe to store on the server and is only used to check my identity. The private key must stay secret with me. If the private key were ever put on the server, anyone with access to that server could steal it and impersonate me.
```

4. Why is the forced proof stronger evidence than a normal successful SSH login?

```text
Because the command turns off password fallback, so the only way it could succeed is with the key. A normal login might have quietly used a password instead. Forcing key-only means the success truly proves the key worked.
```

5. How is the key passphrase different from the `analyst` account password?

```text
The account password logs into the analyst user on the machine. The key passphrase only unlocks my private key file so it can be used. They protect two different things and should not be the same.
```
