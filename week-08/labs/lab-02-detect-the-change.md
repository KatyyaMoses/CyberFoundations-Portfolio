...
```text
No, the SHA 256 digest does not the stay the same. A hash is based on the exact contents of the file, so adding one line changes the input. Therefore, it would be different.
```
...
**[TERMINAL] Run:** cp evidence/incident-report.txt hashes/incident-report-modified.txt echo "Review Note: Integrity validation exercise completed." >> hashes/incident-report-modified.txt tail -n 3 hashes/incident-report-modified.txt
...
**[TERMINAL] Run:** ORIGINAL_HASH=$(cut -d' ' -f1 hashes/original.sha256) MODIFIED_HASH=$(cut -d' ' -f1 hashes/modified.sha256) printf "ORIGINAL: %s\nMODIFIED: %s\n" "$ORIGINAL_HASH" "$MODIFIED_HASH" if [ "$ORIGINAL_HASH" = "$MODIFIED_HASH" ]; then   echo "UNEXPECTED: hashes match - stop and troubleshoot" else   echo "EXPECTED: hashes differ because the file changed" fi
...
```text
The mismatch proves that the file changed after I took the first hash, because adding just one line gave me a completely different SHA-256 value. This shows hashing is good at catching when data has been altered. What it does not prove is who made the change, and it does not prove when the change happened. It only tells me that something is different.
```
...
```text
It proves the file was changed. Since the hash comes from the exact contents of the file, a new value means the data is not the same as the original anymore. So it confirms the file's integrity was broken.
```

2. Why does the mismatch not identify the person who changed the file?

```text
Because the hash only looks at what is in the file, not who touched it. It can tell me that something changed, but it has no idea about the user or the time it happened. To find out who did it, I would need logs or access records.
```

3. Why does hashing not protect confidentiality?

```text
Because hashing does not hide the data. The original file is still there and still readable by anyone who opens it. Hashing just makes a fingerprint to check for changes. If I want to keep the contents private, I need encryption, not hashing.
```
