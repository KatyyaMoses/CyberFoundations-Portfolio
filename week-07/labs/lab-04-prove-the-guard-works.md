...
| --- | --- | --- |
| Grid Beacon `10.60.6.4` | allow | Student are allowed at priority 300 matches this exact source and is before 1000 |
| Other Test Source `10.60.6.10` | deny | the traffic would be denied as it is TCP 8080. This is pasted protected rule 1000 |
...
```text
Ran Test My Rule from Grid Beacon (10.60.6.4). Verdict: ALLOWED. This matches my prediction. My student Allow at priority 300 names this exact source and is read before the protected rule 1000, so it matches first and the traffic gets through.
```
...
```text
Ran the same TCP 8080 test from Other Test Source (10.60.6.10). Verdict: DENIED. This matches my prediction. My Allow only names 10.60.6.4, so 10.60.6.10 doesn't match it. Nothing else allows it, so the traffic falls through to the protected rule 1000, which denies TCP 8080.
```
...
| Is Grid Beacon allowed? | yes, allowed |
| Is Other Test Source denied? | yes, denied |
| Which rule produces the intended Allow? | my student allow at prioriity 300 |
...
![Final rule — week07-lab04-final-rule.png](https://raw.githubusercontent.com/KatyyaMoses/CyberFoundations-Portfolio/074fb61a2933927c17a086bfbf20b6794e962c0f/assets/screenshots/week-07/week07-lab04-final-rule.png)

![Grid Beacon ALLOWED result — week07-lab04-grid-beacon-allowed.png](https://raw.githubusercontent.com/KatyyaMoses/CyberFoundations-Portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab04-grid-beacon-allowed.png)

![Other Test Source DENIED result — week07-lab04-other-source-denied.png](https://raw.githubusercontent.com/KatyyaMoses/CyberFoundations-Portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab04-other-source-denied.png)
...
```text
I configured a single inbound Allow rule at priority 300 that permits TCP traffic on port 8080 from one source only, the host 10.60.6.4. The rule names that exact host rather than the whole subnet or Any, so its scope is deliberately narrow. I then ran two tests against the same port. Grid Beacon at 10.60.6.4 returned ALLOWED, because it matches my rule and that rule is read before the protected priority 1000 deny. The Other Test Source at 10.60.6.10 returned DENIED, because it does not match my rule and falls through to the protected fallback that denies TCP 8080 from the subnet. Together these results show the rule permits exactly the intended host and nothing more. That is least privilege in practice, granting the smallest access that still lets the intended connection work.
```
...
```text
One ALLOWED result on its own only shows that traffic can get through, but it doesn't prove the rule is limited to the host I named. One DENIED result on its own only shows something was blocked, but not that the intended host still works. Together, they show both halves of the story: that the right source gets in and the wrong source is kept out. That combination is what actually proves the rule is narrow instead of just open or just closed.
```
...
```text
First, I would check the Source field on my rule to see if it was accidentally set to the subnet or Any instead of the single host 10.60.6.4. Then I would look at the priority order to make sure no broader Allow sits above the protected deny and is matching first. I would also confirm I hadn't created a second rule that widens the access without meaning to. Only after finding which rule was actually matching would I make any change, since the lab says not to alter things blindly.
```

**Analysis Question 3.** How does this evidence distinguish configuration from observed enforcement? (Minimum 3 sentences.)

```text
The configuration is what the rule says on paper: the source, port, action, and priority I set. The observed enforcement is what actually happened when I ran the tests: one source allowed and one denied. The screenshots of the test results prove the rule isn't just written a certain way but is actually being enforced that way in practice. That's the difference between claiming a rule works and showing that it does.
```
