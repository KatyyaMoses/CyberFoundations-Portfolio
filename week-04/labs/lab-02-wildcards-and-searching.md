...
```
About 11 files. I can see a few naming families:
- invoices (start with inv-)
- access logs (end in -access.log)
- notes/meeting files (notes-, meeting-)
- a supply list
```
...
```
02:47 DENIED badge 4471 storeroom door. It was outside of business hours and that makes it suspicious.
```
...
```
door-access.log (3 matches)
west-access.log (1 match)
...
```
The file was -rw-rw-rw-, meaning the owner, the group, and other could all read and write it. That file held storeroom badge codes. Anyone on the system could read the codes or even change them. Sensitive contents should be owner-only, and this file permission was open to everyone.
```
...
```
ls lets you see what a pattern catches before you act on it. If you skip it and go straight to copying, you might copy the wrong files and not even know it. With rm the stakes are higher, because rm deletes. A bad copy you can clean up. A bad delete is gone for good.
```
...
```
If you have 600 lines to read by hand, you're going to make mistakes and miss things. Grep cuts it down to the three lines that matter. That's easier and safer. Doing that fast is more useful than any one answer, because you'll use it over and over.
```
...
```
Finding the file with grep tells you where the sensitive info is. But seeing it doesn't change anything. The file is still wide open. Checking the permissions tells you if it's protected, and changing them locks it down. Grep finds the risk but can't fix it. A permission check fixes it but can't find it. You need both.
```
...
```
Using patterns and grep to search a lot of files at once. You can't open millions of files one by one. One command sweeps all of them and shows you what matters. That's the habit that still works no matter how big the system gets.
```
